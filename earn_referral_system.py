...
from werkzeug.security import generate_password_hash, check_password_hash  # FIX: hash admin passwords instead of plaintext
...
def init_db():
    ...
    if not query_db("SELECT * FROM admin", one=True):
        # FIX: store a hashed password, never plaintext
        query_db("INSERT INTO admin (username, password) VALUES (?, ?)", (ADMIN_USERNAME, generate_password_hash(ADMIN_PASSWORD)), commit=True)
        ...
        <main class="flex-1 h-full overflow-y-auto p-8 bg-gray-900">
            {% with messages = get_flashed_messages(with_categories=true) %}
                ...
                <div class="mb-6 space-y-2">
                    ...
                    <div class="px-4 py-3 rounded border {{ 'bg-red-900 border-red-700 text-red-200' if category == 'error' else 'bg-green-900 border-green-700 text-green-200' }}">{{ msg }}</div>
                    {% endfor %}
                </div>
                {% endif %}
            {% endwith %}
...
# ==========================================
app = Flask(__name__)
# FIX: a random secret_key generated on every restart invalidates every admin session whenever
# the process restarts (common on Render free tier). Read from env so it's stable across restarts,
# but still fall back to a random key so the app works out-of-the-box.
app.secret_key = os.environ.get("FLASK_SECRET_KEY", os.urandom(24))
if "FLASK_SECRET_KEY" not in os.environ:
    logger.warning("FLASK_SECRET_KEY env var not set — admin sessions will be logged out on every restart. "
                    "Set FLASK_SECRET_KEY for persistent logins.")

# FIX: harden the session cookie. HTTPONLY blocks JS/XSS from stealing the cookie,
# SAMESITE=Lax blocks the cookie being sent on cross-site POSTs (mitigates CSRF on these forms),
# SECURE ensures it's only sent over HTTPS (Render serves HTTPS).
app.config.update(
...
env = Environment(loader=DictLoader(HTML_TEMPLATES), autoescape=select_autoescape(['html']))
# FIX: autoescape was OFF before. Since user-controlled text (Telegram usernames, withdrawal
# bank/UPI/crypto details typed by end users, task/channel fields) is rendered straight into
# these admin templates, this was a stored-XSS hole — anyone could put <script> in a withdraw
# detail field via the bot and have it execute in the admin's browser. Now Jinja escapes by default.
...
# FIX: very small in-memory brute-force lockout. Not a substitute for a real rate limiter,
# but stops trivial unlimited password guessing against the admin login form.
LOGIN_ATTEMPTS = {}
...
def admin_login():
    if request.method == 'POST':
        ...
        if time.time() < locked_until:
            ...
            return render('login.html', error=f"Too many failed attempts. Try again in {wait}s.")
        ...
        if admin:
            try:
                # Normal path: stored password is a werkzeug hash
                valid = check_password_hash(admin['password'], password)
            except ValueError:
                # FIX: legacy plaintext password from before this fix — accept once, then upgrade to a hash
                if admin['password'] == password:
                    valid = True
                    query_db("UPDATE admin SET password=? WHERE username=?", (generate_password_hash(password), username), commit=True)
        ...
        if attempts >= MAX_LOGIN_ATTEMPTS:
            LOGIN_ATTEMPTS[ip] = (attempts, time.time() + LOCKOUT_SECONDS)
            return render('login.html', error="Too many failed attempts. Account locked for 5 minutes.")
...
def admin_edit_balance():
    # FIX: balance was inserted straight from request.form with no validation. SQLite would
    # happily store a non-numeric string in this FLOAT column, and every later
    # "balance = balance + ?" arithmetic operation for that user would then silently fail
    # (or behave unpredictably), effectively breaking the bot for that user.
    try:
        new_balance = float(request.form['balance'])
    except (ValueError, KeyError):
        flash("Balance must be a valid number.", "error")
        return redirect(url_for('admin_users'))
    query_db("UPDATE users SET balance=? WHERE user_id=?", (new_balance, request.form['user_id']), commit=True)
...
def admin_add_channel():
    ...
    # FIX: previously empty strings could be inserted silently, producing broken "Force Join" buttons.
    if not (name and link and channel_id):
        flash("All channel fields are required.", "error")
        return redirect(url_for('admin_channels'))
    query_db("INSERT INTO channels (channel_name, channel_link, channel_id) VALUES (?, ?, ?)",
             (name, link, channel_id), commit=True)
    return redirect(url_for('admin_channels'))
...
def admin_add_task():
    ...
    # FIX: reward was inserted unvalidated — a non-numeric or negative reward would either
    # corrupt the column (breaking balance math when a user completes the task) or let an
    # admin accidentally create a task that deducts coins instead of paying them.
    try:
        reward = float(request.form.get('reward', ''))
        if reward < 0:
            raise ValueError
    except ValueError:
        flash("Reward must be a valid non-negative number.", "error")
        return redirect(url_for('admin_tasks'))
    if not (title and link):
        flash("Title and link are required.", "error")
        return redirect(url_for('admin_tasks'))
    query_db("INSERT INTO tasks (title, reward, link, description) VALUES (?, ?, ?, ?)",
             (title, reward, link, description), commit=True)
    return redirect(url_for('admin_tasks'))
...
def admin_withdraw_action():
    ...
    # FIX: previously any value other than exactly "Approve" silently fell through to the
    # "else" branch and was treated as a Reject (refunding the user) — a typo'd or tampered
    # action value would still trigger a refund. Now we explicitly whitelist the two valid actions.
    if action not in ('Approve', 'Reject'):
        flash("Invalid action.", "error")
        return redirect(url_for('admin_withdraws'))

    w = query_db("SELECT user_id, amount, status FROM withdraw_requests WHERE id=?", (req_id,), one=True)
    if not w:
        flash("Withdrawal request not found.", "error")
        return redirect(url_for('admin_withdraws'))

    # FIX: previously a request could be approved/rejected more than once (e.g. double-click,
    # resubmitted form, or two admins acting at once). Rejecting an already-rejected request a
    # second time would refund the user's balance again. Now we only act on Pending requests.
    if w['status'] != 'Pending':
        flash(f"This request was already {w['status'].lower()}.", "error")
        return redirect(url_for('admin_withdraws'))
...
def admin_save_settings():
    numeric_keys = ['referral_reward', 'daily_bonus', 'min_withdraw']
    text_keys = ['welcome_message', 'bot_token']

    # FIX: these were saved as raw strings with no validation. A non-numeric or negative value
    # here (e.g. typo'd referral_reward) would silently break every reward calculation across
    # the whole bot the next time get_setting(..., float) tried to use it.
    for k in numeric_keys:
        if k in request.form:
            try:
                ...
                flash(f"'{k.replace('_', ' ').title()}' must be a non-negative number. Settings not saved.", "error")
    ...
    # FIX: force_join must only ever be "0" or "1" — anything else breaks the `== '1'` check used elsewhere.
    if 'force_join' in request.form:
        set_setting('force_join', '1' if request.form['force_join'] == '1' else '0')

    new_pass = request.form.get('new_password', '').strip()
    if new_pass:
        # FIX: password was previously stored in plaintext here too.
        admin_username = session.get('admin_username', ADMIN_USERNAME)
        query_db("UPDATE admin SET password=? WHERE username=?", (generate_password_hash(new_pass), admin_username), commit=True)
        flash("Settings saved and password updated.", "success")
    else:
